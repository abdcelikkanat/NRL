...
sys.path.append("./lib")
from updatedgensim.models.keyedvectors import KeyedVectors
...
try:
    ...
    from updatedgensim.models.word2vec_inner import train_batch_sg_topic
...
class Word2Vec(utils.SaveLoad):
    ...
    def _do_train_job_train(self, sentences, alpha, inits):
        """
        Train a single batch of sentences. Return 2-tuple `(effective word count after
        ignoring unknown words and sentence length trimming, total word count)`.
        """
        ...
        if self.sg:
            ...
            raise ValueError("Not implemented for CBOW!")
            tally += train_batch_cbow(self, sentences, alpha, work, neu1, self.compute_loss)
        return tally, self._raw_word_count(sentences)
    ...
    def train_topic(self, number_of_topics, sentences, total_examples=None, total_words=None,
        ...
        self.reset_weights_topic(number_of_topics)
        ...
        if not len(self.wv.syn0_topic):
            raise RuntimeError("you must first finalize topic vocabulary before training the model")
        ...
        def worker_loop():
            ...
            while True:
                ...
                if job is None:
                    ...
                    break  # no more jobs => quit this worker
                sentences, alpha = job
                tally, raw_tally = self._do_train_job_train(sentences, alpha, (work, neu1))
                progress_queue.put((len(sentences), tally, raw_tally))  # report back progress
                jobs_processed += 1
            logger.debug("worker exiting, processed %i jobs", jobs_processed)
    ...
    def reset_weights_topic(self, number_of_topics):
        """Reset all projection weights to an initial (untrained) state, but keep the existing vocabulary."""
        logger.info("resetting layer weights")
        self.wv.syn0_topic = empty((number_of_topics, self.vector_size), dtype=REAL)
        #self.wv.syn0_topic = empty((len(self.wv.vocab), self.vector_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
