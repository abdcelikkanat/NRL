...
cdef unsigned long long fast_sentence_sg_neg_topic(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *syn0, REAL_t *syn1neg, REAL_t *syn0_topic, const int size, const np.uint32_t word_index,
    const np.uint32_t topic_index, const REAL_t alpha, REAL_t *work,
    unsigned long long next_random, REAL_t *word_locks,
    const int _compute_loss, REAL_t *_running_training_loss_param) nogil:
    ...
    # our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0[row1], &ONE)
    our_saxpy(&size, &word_locks[topic_index], work, &ONE, &syn0_topic[row1], &ONE)
...
def train_batch_sg_topic(model, sentences, alpha, _work, compute_loss):
    ...
    cdef REAL_t *word_locks = <REAL_t *>(np.PyArray_DATA(model.syn0_lockf_topic))
    cdef REAL_t *work
    cdef REAL_t _alpha = alpha
    cdef int size = model.layer1_size
