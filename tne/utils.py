...
def find_topics_for_nodes(phi_file, id2node, number_of_nodes, number_of_topics):
...
def find_topics_for_nodes_min(phi_file, id2node, number_of_nodes, number_of_topics):
...
def concatenate_embeddings(node_embedding_file, topic_embedding_file, node2topic, output_filename):
    ...
    with open(topic_embedding_file, 'r') as f:
        ...
        for line in f:
            tokens = line.strip().split()
            # word = int(tokens[0])
            topic_embeddings.update({str(topic_num): [val for val in tokens]})
            topic_num += 1
...
def concatenate_embeddings_avg(node_embedding_file, topic_embedding_file, phi_file, id2node, output_filename):
    ...
    with open(topic_embedding_file, 'r') as f:
        ...
        for line in f:
            tokens = line.strip().split()
            # word = int(tokens[0])
            topic_embeddings.update({unicode(topic_num): [float(val) for val in tokens]})
            topic_num += 1
