...
#from lib.updatedgensim.models_calisan.word2vec import Word2Vec
from gensim.models.word2vec import Word2Vec
...
model = NodeTopic2Vec(walks, size=embedding_size, window=window_size, sg=1, hs=1, workers=workers,
...
combined_sentences = CombineSentence(lda_node_corpus, lda_topic_corpus)
# Extract the topic embeddings
model.train_topic(number_of_topics, combined_sentences)
# Save the topic embeddings
model.wv.save_word2vec_format_topic(fname=topic_embedding_file)
...
initial_time = time.time()
node2topic_min = utils.find_topics_for_nodes_min(phi_file, id2node, number_of_nodes, number_of_topics)
# Concatenate the embeddings
