...
cdef void fast_sentence_sg_hs_topic(
    ...
    our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0_topic[row1], &ONE)
...
def train_batch_sg_topic(model, sentences, alpha, _work, compute_loss):
    ...
    cdef REAL_t *syn0_topic = <REAL_t *>(np.PyArray_DATA(model.wv.vectors))
    ...
    for sent in sentences:
        ...
        for token in sent:
            ...
            indexes_topic[effective_words] = token[1]
            if hs:
            ...
            for i in range(idx_start, idx_end):
                ...
                for j in range(j, k):
                    if j == i:
                        continue
                    if hs:
                        fast_sentence_sg_hs_topic(points[i], codes[i], codelens[i], syn0_topic, syn1, size, indexes_topic[j], _alpha, work, word_locks, _compute_loss, &_running_training_loss)
                    if negative:
                        pass
                        next_random = fast_sentence_sg_neg_topic(negative, cum_table, cum_table_len, syn0_topic, syn1neg, size, indexes[i], indexes_topic[j], _alpha, work, next_random, word_locks, _compute_loss, &_running_training_loss)
